...
public class AppData {
    ...
    public static List<String[]> users = new ArrayList<>(Arrays.asList(
       ...
       new String[]{"cashier","cashier", "1111"}
    ));
    ...
    public static List<String[]> products = new ArrayList<>(Arrays.asList(
        ...
        // ID Laces
        new String[]{"28","ID Lace - BSIT (Blue)","5","35","80","10","8934670028","pcs"},
        new String[]{"29","ID Lace - BSBA (Gold)","5","35","75","10","8934670029","pcs"},
        new String[]{"30","ID Lace - BSED (Green)","5","35","60","10","8934670030","pcs"},
        new String[]{"31","ID Lace - BSN (White)","5","35","55","10","8934670031","pcs"},
        new String[]{"32","ID Lace - BSCRIM (Red)","5","35","50","10","8934670032","pcs"}
    ));
    public static int nextProdId = 33;

    // TRANSACTIONS
    // Each: [id, itemCount, total, date, time, itemsSummary]
    ...
    public static String getCategoryName(String catId) {
        return categories.stream()
            .filter(c -> c[0].equals(catId))
            .map(c -> c[1])
            .findFirst().orElse("—");
    }

    public static int getStock(String[] product) {
        return Integer.parseInt(product[4]);
    }

    public static int getLowStock(String[] product) {
        return Integer.parseInt(product[5]);
    }
    ...
    public static String getUserName(String username) {
        return users.stream().filter(u -> u[1].equals(username))
            .map(u -> u[0]).findFirst().orElse(username);
    }
